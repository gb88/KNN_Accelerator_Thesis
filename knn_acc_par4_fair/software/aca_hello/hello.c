...
#define OVERFLOWREG 0x004C
#define STARTCLREG	0x0054
#define KVALREG 0x0050
...
static volatile int txrx_doneCache2 = 0;
static volatile int txrx_doneCache3 = 0;
static volatile int load_q_cache0 = 0;
static volatile int load_q_cache1 = 0;
static volatile int load_q_cache2 = 0;
static volatile int load_q_cache3 = 0;
...
static void txrxDoneCache2(void * handle, void * data)
{
	txrx_doneCache2 = 1;
	if(load_q_cache2 == 1)
		load_q_cache2 = 1;
}
static void txrxDoneCache3(void * handle, void * data)
{
	txrx_doneCache3 = 1;
	if(load_q_cache3 == 1)
		load_q_cache3 = 1;
...
int main()
{
	...
	while(1)
	{
		...
		IOWR_32DIRECT(NTRREG_2_BASE,0x00,3);
		IOWR_32DIRECT(NTRREG_3_BASE,0x00,3);
		// k val
		...
		IOWR_32DIRECT(ENDTSETREG_2_BASE,0x00,0);
		IOWR_32DIRECT(ENDTSETREG_3_BASE,0x00,0);
		load_q_cache0 = 0;
		load_q_cache1 = 0;
		load_q_cache2 = 0;
		load_q_cache3 = 0;
		...
		IOWR_32DIRECT(FULLREG_2_BASE,0x00,0x00000000);
		IOWR_32DIRECT(FULLREG_3_BASE,0x00,0x00000000);
		...
		txrx_doneCache2 = 0;
		tx_data = (void*)(SDRAM_CONTROLLER_BASE+SDRAM_Q_ADDR_BASE); /* pointer to data to send */
		rx_buffer = (void*)(DMA_WRITE_MASTER_CACHE_2_BASE+cache_q_addr);//CUSTOM_RAM_0_BASE
		alt_dma_txchan_send (txchan,tx_data,q_size*4,NULL,NULL);
		alt_dma_rxchan_prepare (rxchan,rx_buffer,q_size*4, txrxDoneCache2,NULL);
		txrx_doneCache3 = 0;
		tx_data = (void*)(SDRAM_CONTROLLER_BASE+SDRAM_Q_ADDR_BASE); /* pointer to data to send */
		rx_buffer = (void*)(DMA_WRITE_MASTER_CACHE_3_BASE+cache_q_addr);//CUSTOM_RAM_0_BASE
		alt_dma_txchan_send (txchan,tx_data,q_size*4,NULL,NULL);
		alt_dma_rxchan_prepare (rxchan,rx_buffer,q_size*4, txrxDoneCache3,NULL);
		...
		empty2 = IORD_32DIRECT(EMPTYREG_2_BASE,0x00);
		empty3 = IORD_32DIRECT(EMPTYREG_3_BASE,0x00);
		while(txrx_doneCache1==0 || txrx_doneCache1==0 || txrx_doneCache2==0 ||txrx_doneCache3==0);
		while(n_load_count < n_load)
		{
			...
			if(empty2 && (n_load_count< n_load) && txrx_doneCache2) // && !endmem0
			{
				//fill cache 1
				IOWR_32DIRECT(FULLREG_2_BASE,0x00,0x00000000);
				IOWR_32DIRECT(ENDTSETREG_2_BASE,0x00,0); //era 1
				if(n_load_count == n_load-1)
					IOWR_32DIRECT(NTRREG_2_BASE, 0x00,n_cell_last/(4*(q_size+1)));
				else
					IOWR_32DIRECT(NTRREG_2_BASE, 0x00,n_cell/(4*(q_size+1)));
				alt_dma_txchan_ioctl(txchan, ALT_DMA_SET_MODE_32, NULL);
				alt_dma_rxchan_ioctl(rxchan, ALT_DMA_SET_MODE_32, NULL);
				txrx_doneCache2 = 0;
				tx_data = (void*)(SDRAM_CONTROLLER_BASE+memptr); /* pointer to data to send */
				rx_buffer = (void*)(DMA_WRITE_MASTER_CACHE_2_BASE);//CUSTOM_RAM_0_BASE
				if(n_load_count == n_load-1)
				{
					alt_dma_txchan_send (txchan,tx_data,n_cell_last,NULL,NULL);
					alt_dma_rxchan_prepare (rxchan,rx_buffer,n_cell_last, txrxDoneCache2,NULL);
				}
				else
				{
					alt_dma_txchan_send (txchan,tx_data,n_cell,NULL,NULL);
					alt_dma_rxchan_prepare (rxchan,rx_buffer,n_cell, txrxDoneCache2,NULL);
				}
				memptr = memptr + n_cell;
				n_load_count++;
				//IOWR_32DIRECT(FULLREG_2_BASE,0x00,0x00000001);
			}
			if(empty3 && (n_load_count< n_load) && txrx_doneCache3) // && !endmem0
			{
				//fill cache 1
				IOWR_32DIRECT(FULLREG_3_BASE,0x00,0x00000000);
				IOWR_32DIRECT(ENDTSETREG_3_BASE,0x00,0); //era 1
				if(n_load_count == n_load-1)
					IOWR_32DIRECT(NTRREG_3_BASE, 0x00,n_cell_last/(4*(q_size+1)));
				else
					IOWR_32DIRECT(NTRREG_3_BASE, 0x00,n_cell/(4*(q_size+1)));
				alt_dma_txchan_ioctl(txchan, ALT_DMA_SET_MODE_32, NULL);
				alt_dma_rxchan_ioctl(rxchan, ALT_DMA_SET_MODE_32, NULL);
				txrx_doneCache3 = 0;
				tx_data = (void*)(SDRAM_CONTROLLER_BASE+memptr); /* pointer to data to send */
				rx_buffer = (void*)(DMA_WRITE_MASTER_CACHE_3_BASE);//CUSTOM_RAM_0_BASE
				if(n_load_count == n_load-1)
				{
					alt_dma_txchan_send (txchan,tx_data,n_cell_last,NULL,NULL);
					alt_dma_rxchan_prepare (rxchan,rx_buffer,n_cell_last, txrxDoneCache3,NULL);
				}
				else
				{
					alt_dma_txchan_send (txchan,tx_data,n_cell,NULL,NULL);
					alt_dma_rxchan_prepare (rxchan,rx_buffer,n_cell, txrxDoneCache3,NULL);
				}
				memptr = memptr + n_cell;
				n_load_count++;
				//IOWR_32DIRECT(FULLREG_3_BASE,0x00,0x00000001);
			}
			empty0 = IORD_32DIRECT(EMPTYREG_0_BASE,0x00);
			empty1 = IORD_32DIRECT(EMPTYREG_1_BASE,0x00);
			empty2 = IORD_32DIRECT(EMPTYREG_2_BASE,0x00);
		...
		IOWR_32DIRECT(ENDTSETREG_2_BASE,0x00,1); //era 1
		IOWR_32DIRECT(ENDTSETREG_3_BASE,0x00,1); //era 1
		end_class = 0;
		while (!end_class)
